    ...
    internal sealed partial class JsonClassInfo
    {
        ...
        public static ulong GetKey(ReadOnlySpan<byte> name)
        {
            ...
            else
            {
                if (length > 7)
                {
                    key = Unsafe.ReadUnaligned<ulong>(ref reference) & 0xffffffffffffff00L;
                    key |= (ulong)(uint)Math.Min(length, 0xff);
                }
                else
                {
                    key =
                        length > 5 ? ((ulong)Unsafe.ReadUnaligned<uint>(ref reference) << 32) | ((ulong)Unsafe.ReadUnaligned<ushort>(ref Unsafe.Add(ref reference, 4)) << 16) :
                        length > 3 ? ((ulong)Unsafe.ReadUnaligned<uint>(ref reference) << 32) :
                        length > 1 ? ((ulong)Unsafe.ReadUnaligned<ushort>(ref reference) << 48) : 0UL;
                    key |= (ulong)(uint)length;

                    if ((length & 1) != 0)
                    {
                        var offset = length - 1;
                        key |= (ulong)Unsafe.Add(ref reference, offset) << ((7 - offset) * 8);
                    }
                }
            }
            ...
            else
            {
                Debug.Assert(
                    // Verify embedded property name.
                    (name.Length < 1 || name[0] == ((key & ((ulong)0xFF << BitsInByte * 7)) >> BitsInByte * 7)) &&
                    (name.Length < 2 || name[1] == ((key & ((ulong)0xFF << BitsInByte * 6)) >> BitsInByte * 6)) &&
                    (name.Length < 3 || name[2] == ((key & ((ulong)0xFF << BitsInByte * 5)) >> BitsInByte * 5)) &&
                    (name.Length < 4 || name[3] == ((key & ((ulong)0xFF << BitsInByte * 4)) >> BitsInByte * 4)) &&
                    (name.Length < 5 || name[4] == ((key & ((ulong)0xFF << BitsInByte * 3)) >> BitsInByte * 3)) &&
                    (name.Length < 6 || name[5] == ((key & ((ulong)0xFF << BitsInByte * 2)) >> BitsInByte * 2)) &&
                    (name.Length < 7 || name[6] == ((key & ((ulong)0xFF << BitsInByte * 1)) >> BitsInByte * 1)) &&
                    // Verify embedded length.
                    (name.Length >= 0xFF || (key & 0xFF) == (ulong)name.Length) &&
                    (name.Length < 0xFF || (key & 0xFF) == 0xFF));
            }
